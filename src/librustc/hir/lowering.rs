...
macro_rules! hir_vec {
    ($this:expr; $elem:expr; $n:expr) => (
        $crate::hir::HirVec::from_slice($this.arena, &[$elem; $n])
    );
    ($this:expr; $($x:expr),*) => (
        $crate::hir::HirVec::from_slice($this.arena, &[$($x),*])
    );
    () => (
        $crate::hir::HirVec::new()
    );
}

macro_rules! thin_hir_vec {
    () => (
        P::empty_thin()
    );
}

pub struct LoweringContext<'r, 'a> {
    ...
    resolver: &'r mut dyn Resolver<'a>,

    /// The items being lowered are collected here.
    items: BTreeMap<NodeId, hir::Item<'a>>,

    trait_items: BTreeMap<hir::TraitItemId, hir::TraitItem<'a>>,
    impl_items: BTreeMap<hir::ImplItemId, hir::ImplItem<'a>>,
    bodies: BTreeMap<hir::BodyId, hir::Body<'a>>,
    exported_macros: Vec<hir::MacroDef<'a>>,
...
#[derive(Debug)]
enum ImplTraitContext<'a, 'hir> {
    /// Treat `impl Trait` as shorthand for a new universal generic parameter.
    /// Example: `fn foo(x: impl Debug)`, where `impl Debug` is conceptually
    /// equivalent to a fresh universal parameter like `fn foo<T: Debug>(x: T)`.
    ///
    /// Newly generated parameters should be inserted into the given `Vec`.
    Universal(&'a mut Vec<hir::GenericParam<'hir>>),
...
impl<'a, 'hir> ImplTraitContext<'a, 'hir> {
    #[inline]
    fn disallowed() -> Self {
        ImplTraitContext::Disallowed(ImplTraitPosition::Other)
    }

    fn reborrow(&'b mut self) -> ImplTraitContext<'b, 'hir> {
...
impl<'a> LoweringContext<'_, 'a> {
    fn lower_crate(mut self, c: &Crate) -> hir::Crate<'a> {
        /// Full-crate AST visitor that inserts into a fresh
        /// `LoweringContext` any information that may be
        /// needed from arbitrary locations in the crate,
        /// e.g., the number of lifetime generic parameters
        /// declared for every type and trait definition.
        struct MiscCollector<'l, 'lcx, 'interner> {
            lctx: &'l mut LoweringContext<'lcx, 'interner>,
        }

        impl<'a, 'l, 'lcx, 'interner> Visitor<'a> for MiscCollector<'l, 'lcx, 'interner> {
        ...
        struct ItemLowerer<'l, 'lcx, 'interner: 'lcx> {
            lctx: &'l mut LoweringContext<'lcx, 'interner>,
        }

        impl<'l, 'lcx, 'interner> ItemLowerer<'l, 'lcx, 'interner> {
        ...
        impl<'a, 'l, 'lcx, 'interner> Visitor<'a> for ItemLowerer<'l, 'lcx, 'interner> {
            fn visit_item(&mut self, item: &'a Item) {
        ...
        hir::Crate {
            ...
            attrs,
            span: c.span,
            exported_macros: P::from_slice(self.arena, &self.exported_macros),
            items: self.items,
    ...
    fn record_body(&mut self, value: hir::Expr<'a>, decl: Option<&FnDecl>) -> hir::BodyId {
        let arena = self.arena;
        let body = hir::Body {
            arguments: decl.map_or(hir_vec![], |decl| {
                decl.inputs.iter().map(|x| self.lower_arg(x)).collect_hir_vec(arena)
            }),
    ...
    fn collect_in_band_defs<T, F>(
        ...
        anonymous_lifetime_mode: AnonymousLifetimeMode,
        f: F,
    ) -> (Vec<hir::GenericParam<'a>>, T)
    where
        F: FnOnce(&mut LoweringContext<'_, 'a>) -> (Vec<hir::GenericParam<'a>>, T),
    {
    ...
    where
        F: FnOnce(&mut LoweringContext<'_, 'a>) -> T,
    {
    ...
    where
        F: FnOnce(&mut LoweringContext<'_, 'a>, &mut Vec<hir::GenericParam<'a>>) -> T,
    {
    ...
    fn make_async_expr(
        ...
        ret_ty: Option<&Ty>,
        body: impl FnOnce(&mut LoweringContext<'_, 'a>) -> hir::Expr<'a>,
    ) -> hir::ExprKind<'a> {
        let prev_is_generator = mem::replace(&mut self.is_generator, true);
        let body_expr = body(self);
        let span = body_expr.span;
        let output = match ret_ty {
            Some(ty) => FunctionRetTy::Ty(syntax::ptr::P(ty.clone())),
        ...
        let gen_future = self.expr_std_path(
            unstable_span, &["future", "from_generator"], None, thin_hir_vec![]);
        hir::ExprKind::Call(p!(self, gen_future), hir_vec![self; generator])
    }

    fn lower_body<F>(&mut self, decl: Option<&FnDecl>, f: F) -> hir::BodyId
    where
        F: FnOnce(&mut LoweringContext<'_, 'a>) -> hir::Expr<'a>,
    {
    ...
    fn copy_attrs<I: IntoIterator<Item=Attribute>>(
        &mut self,
        attrs: I
    ) -> hir::ThinHirVec<'a, Attribute> {
        let vec = P::from_existing(self.arenas.attrs.alloc_from_iter(attrs));
        if vec.is_empty() {
            P::empty_thin()
        } else {
            P::alloc(self.arena, vec)
        }
    }

    fn lower_attrs(&mut self, attrs: &[Attribute]) -> hir::HirVec<'a, Attribute> {
        P::from_existing(
            self.arenas.attrs.alloc_from_iter(attrs.iter().map(|a| self.lower_attr(a)))
        )
    }
    ...
    fn lower_arm(&mut self, arm: &Arm) -> hir::Arm<'a> {
        let arena = self.arena;
        hir::Arm {
            attrs: self.lower_attrs(&arm.attrs),
            pats: arm.pats.iter().map(|x| self.lower_pat(x)).collect_hir_vec(arena),
            guard: match arm.guard {
                Some(Guard::If(ref x)) => Some(hir::Guard::If(p!(self, self.lower_expr(x)))),
                _ => None,
            },
            body: p!(self, self.lower_expr(&arm.body)),
        }
    }

    fn lower_ty_binding(&mut self, b: &TypeBinding,
                        itctx: ImplTraitContext<'_, 'a>) -> hir::TypeBinding<'a> {
        hir::TypeBinding {
    ...
    fn lower_ty(&mut self, t: &Ty, itctx: ImplTraitContext<'_, 'a>) -> P<'a, hir::Ty<'a>> {
        p!(self, self.lower_ty_direct(t, itctx))
    }

    fn lower_ty_direct(&mut self, t: &Ty, mut itctx: ImplTraitContext<'_, 'a>) -> hir::Ty<'a> {
        let arena = self.arena;
        let kind = match t.node {
            ...
            TyKind::BareFn(ref f) => self.with_in_scope_lifetime_defs(
                ...
                |this| {
                    this.with_anonymous_lifetime_mode(
                        ...
                        |this| {
                            hir::TyKind::BareFn(p!(this, hir::BareFnTy {
            ...
            TyKind::ImplicitSelf => hir::TyKind::Path(hir::QPath::Resolved(
                ...
                p!(self, hir::Path {
                    ...
                    segments: hir_vec![
                        self;
                        hir::PathSegment::from_ident(keywords::SelfUpper.ident())
                    ],
                    span: t.span,
                }),
            )),
                    ...
                    ImplTraitContext::Universal(in_band_ty_params) => {
                        ...
                        hir::TyKind::Path(hir::QPath::Resolved(
                            ...
                            p!(self, hir::Path {
                                ...
                                segments: hir_vec![self; hir::PathSegment::from_ident(ident)],
                            }),
                        ))
                    }
    ...
    fn lower_existential_impl_trait(
        ...
        lower_bounds: impl FnOnce(&mut LoweringContext<'_, 'a>) -> hir::GenericBounds<'a>,
    ) -> hir::TyKind<'a> {
    ...
    fn lifetimes_from_impl_trait_bounds(
    ...
    ) -> (HirVec<'a, hir::GenericArg<'a>>, HirVec<'a, hir::GenericParam<'a>>) {
        // This visitor walks over impl trait bounds and creates defs for all lifetimes which
        // appear in the bounds, excluding lifetimes that are created within the bounds.
        // E.g., `'a`, `'b`, but not `'c` in `impl for<'c> SomeTrait<'a, 'b, 'c>`.
        struct ImplTraitLifetimeCollector<'l, 'r, 'a> {
            context: &'l mut LoweringContext<'r, 'a>,
            parent: DefIndex,
            exist_ty_id: NodeId,
            collect_elided_lifetimes: bool,
            currently_bound_lifetimes: Vec<hir::LifetimeName>,
            already_defined_lifetimes: FxHashSet<hir::LifetimeName>,
            output_lifetimes: Vec<hir::GenericArg<'a>>,
            output_lifetime_params: Vec<hir::GenericParam<'a>>,
        }

        impl<'v> hir::intravisit::Visitor<'v> for ImplTraitLifetimeCollector<'_, '_, '_> {
        ...
        (
            P::from_slice(arena, &lifetime_collector.output_lifetimes),
            P::from_slice(arena, &lifetime_collector.output_lifetime_params),
        )
    }

    fn lower_foreign_mod(&mut self, fm: &ForeignMod) -> hir::ForeignMod<'a> {
        let arena = self.arena;
        hir::ForeignMod {
            abi: fm.abi,
            items: fm.items
    ...
    fn lower_global_asm(&mut self, ga: &GlobalAsm) -> P<'a, hir::GlobalAsm> {
        p!(self, hir::GlobalAsm {
            asm: ga.asm,
            ctxt: ga.ctxt,
        })
    }

    fn lower_variant(&mut self, v: &Variant) -> hir::Variant<'a> {
        Spanned {
    ...
    fn lower_qpath(
        ...
        let mut ty = if path.segments.is_empty() {
            ...
            p!(self, self.ty_path(new_id, p.span, hir::QPath::Resolved(qself, path)))
        };
        ...
        for (i, segment) in p.segments.iter().enumerate().skip(proj_start) {
            let segment = p!(self, self.lower_path_segment(
                p.span,
                segment,
                param_mode,
                0,
            ...
            let new_id = self.next_id();
            ty = p!(self, self.ty_path(new_id, p.span, qpath));
        }
    ...
    fn lower_path(&mut self, id: NodeId, p: &Path, param_mode: ParamMode) -> hir::Path<'a> {
    ...
    fn lower_parenthesized_parameter_data(
        ...
        self.with_anonymous_lifetime_mode(
            ...
            |this| {
                ...
                (
                    hir::GenericArgs {
                        args: hir_vec![this; GenericArg::Type(mk_tup(this, inputs, span))],
                        bindings: hir_vec![
                            this;
                            hir::TypeBinding {
                                ...
                                ty: output
                                    ...
                                    .unwrap_or_else(|| p!(
                                        this,
                                        mk_tup(this, hir::HirVec::new(), span)
                                    )),
    ...
    fn lower_local(&mut self, l: &Local) -> (P<'a, hir::Local<'a>>, SmallVec<[hir::ItemId; 1]>) {
        ...
        (p!(self, hir::Local {
            ...
            init: l.init.as_ref().map(|e| p!(self, self.lower_expr(e))),
            span: l.span,
            attrs: self.copy_attrs(l.attrs.iter().cloned()),
    ...
    fn lower_arg(&mut self, arg: &Arg) -> hir::Arg<'a> {
    ...
    fn lower_fn_args_to_names(&mut self, decl: &FnDecl) -> hir::HirVec<'a, Ident> {
        decl.inputs
            .iter()
    ...
    fn lower_fn_decl(
        &mut self,
        decl: &FnDecl,
        mut in_band_ty_params: Option<(DefId, &mut Vec<hir::GenericParam<'a>>)>,
    ...
    fn lower_async_fn_ret_ty(
        ...
        struct AsyncFnLifetimeCollector<'l, 'r, 'a> {
            context: &'l mut LoweringContext<'r, 'a>,
        ...
        impl<'v> hir::intravisit::Visitor<'v> for AsyncFnLifetimeCollector<'_, '_, '_> {
            fn nested_visit_map<'this>(
                &'this mut self,
            ) -> hir::intravisit::NestedVisitorMap<'this, 'v> {
                hir::intravisit::NestedVisitorMap::None
        ...
        let impl_trait_ty = self.lower_existential_impl_trait(
            ...
            // "<Output = T>"
            let future_params = p!(this, hir::GenericArgs {
                args: hir_vec![],
                bindings: hir_vec![this; hir::TypeBinding {
                    ident: Ident::from_str(FN_OUTPUT_NAME),
                    ty: output_ty,
                    id: this.next_id().node_id,
                    span,
            ...
            P::from_slice(arena, &bounds)
        });

        let LoweredNodeId { node_id, hir_id } = self.next_id();
        let impl_trait_ty = p!(self, hir::Ty {
            id: node_id,
            node: impl_trait_ty,
            span,
            hir_id,
    ...
    fn lower_generic_params(
        ...
        mut itctx: ImplTraitContext<'_, 'a>,
    ) -> hir::HirVec<'a, hir::GenericParam<'a>> {
        let arena = self.arena;
    ...
    fn lower_where_clause(&mut self, wc: &WhereClause) -> hir::WhereClause<'a> {
        let arena = self.arena;
        self.with_anonymous_lifetime_mode(
            AnonymousLifetimeMode::ReportError,
            |this| {
    ...
    fn lower_where_predicate(&mut self, pred: &WherePredicate) -> hir::WherePredicate<'a> {
        let arena = self.arena;
        match *pred {
    ...
    fn lower_variant_data(&mut self, vdata: &VariantData) -> hir::VariantData<'a> {
        let arena = self.arena;
        match *vdata {
    ...
    fn lower_struct_field(&mut self, (index, f): (usize, &StructField)) -> hir::StructField<'a> {
        hir::StructField {
            span: f.span,
    ...
    fn lower_field(&mut self, f: &Field) -> hir::Field<'a> {
        hir::Field {
            id: self.next_id().node_id,
            ident: f.ident,
            expr: p!(self, self.lower_expr(&f.expr)),
            span: f.span,
            is_shorthand: f.is_shorthand,
        }
    }

    fn lower_mt(&mut self, mt: &MutTy, itctx: ImplTraitContext<'_, 'a>) -> hir::MutTy<'a> {
        hir::MutTy {
    ...
    fn lower_param_bounds(&mut self, bounds: &[GenericBound], mut itctx: ImplTraitContext<'_, 'a>)
        -> hir::GenericBounds<'a> {
        let arena = self.arena;
        bounds.iter().map(|bound| self.lower_param_bound(bound, itctx.reborrow()))
                     .collect_hir_vec(arena)
    }

    fn lower_block(&mut self, b: &Block, targeted_by_break: bool) -> P<'a, hir::Block<'a>> {
        let mut expr = None;
        ...
        for (index, stmt) in b.stmts.iter().enumerate() {
            if index == b.stmts.len() - 1 {
                if let StmtKind::Expr(ref e) = stmt.node {
                    expr = Some(p!(self, self.lower_expr(e)));
                } else {
        ...
        p!(self, hir::Block {
            id: node_id,
            hir_id,
            stmts: P::from_slice(self.arena, &stmts),
            expr,
            ...
            if let IsAsync::Async { closure_id, .. } = asyncness {
                let async_expr = this.make_async_expr(
                    ...
                    |this| {
                        ...
                        this.expr_block(body, thin_hir_vec![])
                    });
                this.expr(body.span, async_expr, thin_hir_vec![])
            } else {
                let body = this.lower_block(body, false);
                this.expr_block(body, thin_hir_vec![])
            }
    ...
    fn lower_item_kind(
        ...
        id: NodeId,
        ident: &mut Ident,
        attrs: &hir::HirVec<'a, Attribute>,
        vis: &mut hir::Visibility<'a>,
        i: &ItemKind,
    ) -> hir::ItemKind<'a> {
        let arena = self.arena;
        match *i {
    ...
    fn lower_use_tree(
        ...
        match tree.kind {
                ...
                *ident = tree.ident();
                ...
                for (def, &new_node_id) in defs.zip([id1, id2].iter()) {
                    ...
                    self.with_hir_id_owner(new_node_id, |this| {
                        ...
                        let item = hir::ItemKind::Use(p!(this, path), hir::UseKind::Single);
                ...
                let path =
                    p!(self, self.lower_path_extra(ret_def, &path, ParamMode::Explicit, None));
                hir::ItemKind::Use(path, hir::UseKind::Single)
            }
            UseTreeKind::Glob => {
                let path = p!(self, self.lower_path(
                    id,
                    &Path {
                        segments,
                        span: path.span,
            ...
            UseTreeKind::Nested(ref trees) => {
                ...
                let path = p!(self, self.lower_path_extra(def, &prefix, ParamMode::Explicit, None));
    ...
    fn renumber_segment_ids(&mut self, path: &P<'a, hir::Path>) -> P<'a, hir::Path<'a>> {
        ...
        path.segments = P::from_slice(self.arena, &segments);
        p!(self, path)
    }

    fn lower_trait_item(&mut self, i: &TraitItem) -> hir::TraitItem<'a> {
    ...
    fn lower_impl_item(&mut self, i: &ImplItem) -> hir::ImplItem<'a> {
        let LoweredNodeId { node_id, hir_id } = self.lower_node_id(i.id);
    ...
    fn lower_impl_item_ref(&mut self, i: &ImplItem) -> hir::ImplItemRef<'a> {
        hir::ImplItemRef {
    ...
    fn lower_mod(&mut self, m: &Mod) -> hir::Mod<'a> {
        let arena = self.arena;
        hir::Mod {
            inner: m.inner,
            item_ids: m.items.iter().flat_map(|x| self.lower_item_id(x)).collect_hir_vec(arena),
        }
    }
    ...
    pub fn lower_item(&mut self, i: &Item) -> Option<hir::Item<'a>> {
        ...
        if let ItemKind::MacroDef(ref def) = i.node {
            if !def.legacy || attr::contains_name(&i.attrs, "macro_export") ||
                ...
                self.exported_macros.push(hir::MacroDef {
                    ...
                    id: i.id,
                    span: i.span,
                    body: P::from_existing(self.arenas.token_streams.alloc(body)),
                    legacy: def.legacy,
                });
            }
            return None;
    ...
    fn lower_foreign_item(&mut self, i: &ForeignItem) -> hir::ForeignItem<'a> {
    ...
    ) -> (hir::Generics<'a>, hir::MethodSig<'a>) {
    ...
    fn lower_pat(&mut self, p: &Pat) -> P<'a, hir::Pat<'a>> {
        let arena = self.arena;
        let node = match p.node {
            ...
            PatKind::Lit(ref e) => hir::PatKind::Lit(p!(self, self.lower_expr(e))),
            PatKind::TupleStruct(ref path, ref pats, ddpos) => {
                ...
                hir::PatKind::TupleStruct(
                    qpath,
                    pats.iter().map(|x| self.lower_pat(x)).collect_hir_vec(arena),
                    ddpos,
                )
            }
            ...
            PatKind::Tuple(ref elts, ddpos) => {
                hir::PatKind::Tuple(elts.iter().map(|x| self.lower_pat(x))
                                        .collect_hir_vec(arena), ddpos)
            }
            ...
            PatKind::Range(ref e1, ref e2, Spanned { node: ref end, .. }) => hir::PatKind::Range(
                p!(self, self.lower_expr(e1)),
                p!(self, self.lower_expr(e2)),
                self.lower_range_end(end),
            ),
            PatKind::Slice(ref before, ref slice, ref after) => hir::PatKind::Slice(
                before.iter().map(|x| self.lower_pat(x)).collect_hir_vec(arena),
                slice.as_ref().map(|x| self.lower_pat(x)),
                after.iter().map(|x| self.lower_pat(x)).collect_hir_vec(arena),
            ),
    ...
    fn lower_expr(&mut self, e: &Expr) -> hir::Expr<'a> {
        let arena = self.arena;
        let kind = match e.node {
            ExprKind::Box(ref inner) => hir::ExprKind::Box(p!(self, self.lower_expr(inner))),
            ExprKind::ObsoleteInPlace(..) => {
                self.sess.abort_if_errors();
                span_bug!(e.span, "encountered ObsoleteInPlace expr during lowering");
            }
            ExprKind::Array(ref exprs) => {
                hir::ExprKind::Array(exprs.iter().map(|x| self.lower_expr(x))
                                                 .collect_hir_vec(arena))
            }
            ExprKind::Repeat(ref expr, ref count) => {
                let expr = p!(self, self.lower_expr(expr));
                let count = self.lower_anon_const(count);
                hir::ExprKind::Repeat(expr, count)
            }
            ExprKind::Tup(ref elts) => {
                hir::ExprKind::Tup(elts.iter().map(|x| self.lower_expr(x)).collect_hir_vec(arena))
            }
            ExprKind::Call(ref f, ref args) => {
                let f = p!(self, self.lower_expr(f));
                hir::ExprKind::Call(
                    f,
                    args.iter().map(|x| self.lower_expr(x)).collect_hir_vec(arena)
                )
            }
            ExprKind::MethodCall(ref seg, ref args) => {
                ...
                let args = args.iter().map(|x| self.lower_expr(x)).collect_hir_vec(arena);
                hir::ExprKind::MethodCall(hir_seg, seg.ident.span, args)
            }
            ExprKind::Binary(binop, ref lhs, ref rhs) => {
                let binop = self.lower_binop(binop);
                let lhs = p!(self, self.lower_expr(lhs));
                let rhs = p!(self, self.lower_expr(rhs));
                hir::ExprKind::Binary(binop, lhs, rhs)
            }
            ExprKind::Unary(op, ref ohs) => {
                let op = self.lower_unop(op);
                let ohs = p!(self, self.lower_expr(ohs));
                hir::ExprKind::Unary(op, ohs)
            }
            ExprKind::Lit(ref l) => {
                let lit = P::from_existing(self.arenas.lits.alloc(l.clone()));
                ...
                let else_opt = else_opt.as_ref().map(|els| {
                    match els.node {
                        ExprKind::IfLet(..) => {
                            ...
                            let span = els.span;
                            let els = p!(self, self.lower_expr(els));
                            let LoweredNodeId { node_id, hir_id } = self.next_id();
                            let blk = p!(self, hir::Block {
                                stmts: hir_vec![],
                                expr: Some(els),
                                id: node_id,
                                hir_id,
                            ...
                            p!(self, self.expr_block(blk, thin_hir_vec![]))
                        }
                        _ => p!(self, self.lower_expr(els)),
                    }
                });

                let then_blk = self.lower_block(blk, false);
                let then_expr = self.expr_block(then_blk, thin_hir_vec![]);

                hir::ExprKind::If(p!(self, self.lower_expr(cond)), p!(self, then_expr), else_opt)
            }
            ExprKind::While(ref cond, ref body, opt_label) => self.with_loop_scope(e.id, |this| {
                hir::ExprKind::While(
                    this.with_loop_condition_scope(|this| p!(this, this.lower_expr(cond))),
            ...
            ExprKind::TryBlock(ref body) => {
                self.with_catch_scope(body.id, |this| {
                    ...
                    let tail = block.expr.take().map_or_else(
                        ...
                        |x: P<'a, hir::Expr<'a>>| x.into_inner(),
                    );
                    block.expr = Some(this.wrap_in_try_constructor(
                        "from_ok", tail, unstable_span));
                    hir::ExprKind::Block(p!(this, block), None)
                })
            }
            ExprKind::Match(ref expr, ref arms) => hir::ExprKind::Match(
                p!(self, self.lower_expr(expr)),
                arms.iter().map(|x| self.lower_arm(x)).collect_hir_vec(arena),
            ...
            ExprKind::Async(capture_clause, closure_node_id, ref block) => {
                self.make_async_expr(capture_clause, closure_node_id, None, |this| {
                    this.with_new_scopes(|this| {
                        ...
                        this.expr_block(block, thin_hir_vec![])
                    })
                })
            }
            ExprKind::Closure(
                ...
                if let IsAsync::Async { closure_id, .. } = asyncness {
                    ...
                    self.with_new_scopes(|this| {
                        ...
                        let body_id = this.lower_body(Some(&outer_decl), |this| {
                            ...
                            this.expr(fn_decl_span, async_body, thin_hir_vec![])
                        });
            ...
            ExprKind::Assign(ref el, ref er) => {
                hir::ExprKind::Assign(p!(self, self.lower_expr(el)), p!(self, self.lower_expr(er)))
            }
            ExprKind::AssignOp(op, ref el, ref er) => hir::ExprKind::AssignOp(
                self.lower_binop(op),
                p!(self, self.lower_expr(el)),
                p!(self, self.lower_expr(er)),
            ),
            ExprKind::Field(ref el, ident) => {
                hir::ExprKind::Field(p!(self, self.lower_expr(el)), ident)
            }
            ExprKind::Index(ref el, ref er) => {
                hir::ExprKind::Index(p!(self, self.lower_expr(el)), p!(self, self.lower_expr(er)))
            }
            ...
            ExprKind::Range(Some(ref e1), Some(ref e2), RangeLimits::Closed) => {
                ...
                let ty_path = p!(
                    self,
                    self.std_path(e.span, &["ops", "RangeInclusive"], None, false)
                );
                let ty = p!(self, self.ty_path(id, e.span, hir::QPath::Resolved(None, ty_path)));
                let new_seg = p!(self, hir::PathSegment::from_ident(Ident::from_str("new")));
                let new_path = hir::QPath::TypeRelative(ty, new_seg);
                let new = p!(
                    self,
                    self.expr(e.span, hir::ExprKind::Path(new_path), thin_hir_vec![])
                );
                hir::ExprKind::Call(new, hir_vec![self; e1, e2])
            }
            ExprKind::Range(ref e1, ref e2, lims) => {
                ...
                let fields = e1.iter()
                    ...
                    .map(|(s, e)| {
                        let expr = p!(self, self.lower_expr(&e));
                ...
                let struct_path = hir::QPath::Resolved(None, p!(self, struct_path));
            ...
            ExprKind::Break(opt_label, ref opt_expr) => {
                ...
                hir::ExprKind::Break(
                    destination,
                    opt_expr.as_ref().map(|x| p!(self, self.lower_expr(x))),
                )
            }
            ...
            ExprKind::Ret(ref e) => {
                hir::ExprKind::Ret(e.as_ref().map(|x| p!(self, self.lower_expr(x))))
            }
            ExprKind::InlineAsm(ref asm) => {
                ...
                let inputs = asm.inputs
                    ...
                    .collect_hir_vec(arena);
                let hir_asm = P::from_existing(self.arenas.inline_asms.alloc(hir_asm));
                hir::ExprKind::InlineAsm(hir_asm, outputs, inputs)
            }
            ExprKind::Struct(ref path, ref fields, ref maybe_expr) => hir::ExprKind::Struct(
                ...
                fields.iter().map(|x| self.lower_field(x)).collect_hir_vec(arena),
                maybe_expr.as_ref().map(|x| p!(self, self.lower_expr(x))),
            ),
            ExprKind::Paren(ref ex) => {
                ...
                if !e.attrs.is_empty() {
                    ex.attrs = self.copy_attrs(
                        e.attrs.iter().cloned().chain(ex.attrs.iter().cloned())
                    );
                }
                return ex;
            }
            ...
            ExprKind::Yield(ref opt_expr) => {
                ...
                let expr = opt_expr
                    ...
                    .unwrap_or_else(||
                    self.expr(e.span, hir::ExprKind::Tup(hir_vec![]), thin_hir_vec![])
                );
                hir::ExprKind::Yield(p!(self, expr))
            }
            ...
            ExprKind::IfLet(ref pats, ref sub_expr, ref body, ref else_opt) => {
                ...
                // `<pat> => <body>`
                {
                    ...
                    let body_expr = p!(self, self.expr_block(body, thin_hir_vec![]));
                    let pats = pats.iter().map(|pat| self.lower_pat(pat)).collect_hir_vec(arena);
                ...
                // _ => [<else_opt>|()]
                {
                    ...
                    let body = if let Some(else_expr) = wildcard_arm {
                        p!(self, self.lower_expr(else_expr))
                    } else {
                        self.expr_tuple(e.span, hir_vec![])
                    };
                    arms.push(self.arm(hir_vec![self; wildcard_pattern], body));
                }

                let contains_else_clause = else_opt.is_some();

                let sub_expr = p!(self, self.lower_expr(sub_expr));

                hir::ExprKind::Match(
                    sub_expr,
                    P::from_slice(self.arena, &arms),
            ...
            ExprKind::WhileLet(ref pats, ref sub_expr, ref body, opt_label) => {
                ...
                let (body, break_expr, sub_expr) = self.with_loop_scope(e.id, |this| {
                    (
                        this.lower_block(body, false),
                        this.expr_break(e.span, thin_hir_vec![]),
                        this.with_loop_condition_scope(|this| p!(this, this.lower_expr(sub_expr))),
                    )
                });
                ...
                let break_arm = {
                    ...
                    self.arm(hir_vec![self; pat_under], break_expr)
                };

                // `match <sub_expr> { ... }`
                let arms = hir_vec![self; pat_arm, break_arm];
                ...
                // `[opt_ident]: loop { ... }`
                let loop_block = p!(self, self.block_expr(p!(self, match_expr)));
            ...
            ExprKind::ForLoop(ref pat, ref head, ref body, opt_label) => {
                ...
                let pat_arm = {
                    ...
                    let val_expr = p!(self, self.expr_ident(pat.span, val_ident, val_pat.id));
                    let next_expr = p!(self, self.expr_ident(pat.span, next_ident, next_pat.id));
                    let assign = p!(self, self.expr(
                        pat.span,
                        hir::ExprKind::Assign(next_expr, val_expr),
                        thin_hir_vec![],
                    ));
                    let some_pat = self.pat_some(pat.span, val_pat);
                    self.arm(hir_vec![self; some_pat], assign)
                };

                // `::std::option::Option::None => break`
                let break_arm = {
                    let break_expr =
                        self.with_loop_scope(e.id, |this| this.expr_break(e.span, thin_hir_vec![]));
                    let pat = self.pat_none(e.span);
                    self.arm(hir_vec![self; pat], break_expr)
                };
                ...
                // `match ::std::iter::Iterator::next(&mut iter) { ... }`
                let match_expr = {
                    let iter = p!(self, self.expr_ident(head_sp, iter, iter_pat.id));
                    let ref_mut_iter = self.expr_mut_addr_of(head_sp, iter);
                    let next_path = &["iter", "Iterator", "next"];
                    let next_path = p!(
                        self,
                        self.expr_std_path(head_sp, next_path, None, thin_hir_vec![])
                    );
                    let next_expr = p!(
                        self,
                        self.expr_call(head_sp, next_path, hir_vec![self; ref_mut_iter])
                    );
                    let arms = hir_vec![self; pat_arm, break_arm];
                ...
                let next_expr = p!(self, self.expr_ident(head_sp, next_ident, next_pat.id));
                ...
                let body_expr = p!(self, self.expr_block(body_block, thin_hir_vec![]));
                let body_stmt = respan(
                    body.span,
                    hir::StmtKind::Expr(body_expr, self.next_id().node_id)
                );

                let loop_block = p!(self, self.block_all(
                    e.span,
                ...
                // `mut iter => { ... }`
                let iter_arm = self.arm(hir_vec![self; iter_pat], loop_expr);

                // `match ::std::iter::IntoIterator::into_iter(<head>) { ... }`
                let into_iter_expr = {
                    let into_iter_path = &["iter", "IntoIterator", "into_iter"];
                    let into_iter = p!(self, self.expr_std_path(
                            head_sp, into_iter_path, None, thin_hir_vec![]));
                    p!(self, self.expr_call(head_sp, into_iter, hir_vec![self; head]))
                };

                let match_expr = p!(self, self.expr_match(
                    head_sp,
                    into_iter_expr,
                    hir_vec![self; iter_arm],
                    hir::MatchSource::ForLoopDesugar,
                ));
                ...
                let result = p!(self, self.expr_ident(e.span, result_ident, let_stmt_binding));
                let block = p!(
                    self,
                    self.block_all(e.span, hir_vec![self; let_stmt], Some(result))
                );
                // Add the attributes to the outer returned expr node.
                let attrs = self.copy_attrs(e.attrs.iter().cloned());
            ...
            ExprKind::Try(ref sub_expr) => {
                ...
                let discr = {
                    ...
                    let path = p!(self, self.expr_std_path(
                            unstable_span, path, None, thin_hir_vec![]));
                    p!(self, self.expr_call(e.span, path, hir_vec![self; sub_expr]))
                };
                ...
                let attrs = self.copy_attrs(Some(attr));

                // `Ok(val) => #[allow(unreachable_code)] val,`
                let ok_arm = {
                    let val_ident = self.str_to_ident("val");
                    let val_pat = self.pat_ident(e.span, val_ident);
                    let val_expr = p!(self, self.expr_ident_with_attrs(
                        e.span,
                        val_ident,
                        val_pat.id,
                        attrs,
                    ...
                    self.arm(hir_vec![self; ok_pat], val_expr)
                };
                ...
                let err_arm = {
                    ...
                    let from_expr = {
                        ...
                        let from = p!(self, self.expr_std_path(
                                e.span, path, None, thin_hir_vec![]));
                        let err_expr = self.expr_ident(e.span, err_ident, err_local.id);

                        self.expr_call(e.span, from, hir_vec![self; err_expr])
                    };
                    let from_err_expr =
                    ...
                    let ret_expr = if let Some(catch_node) = catch_scope {
                        ...
                        p!(self, self.expr(e.span, hir::ExprKind::Ret(Some(from_err_expr)), attrs))
                    };

                    let err_pat = self.pat_err(e.span, err_local);
                    self.arm(hir_vec![self; err_pat], ret_expr)
                };

                hir::ExprKind::Match(
                    discr,
                    hir_vec![self; err_arm, ok_arm],
        ...
        hir::Expr {
            ...
            node: kind,
            span: e.span,
            attrs: self.copy_attrs(e.attrs.iter().cloned()),
        }
    }

    fn lower_stmt(&mut self, s: &Stmt) -> SmallVec<[hir::Stmt<'a>; 1]> {
        smallvec![match s.node {
            ...
            StmtKind::Expr(ref e) => Spanned {
                node: hir::StmtKind::Expr(
                    p!(self, self.lower_expr(e)),
                    self.lower_node_id(s.id).node_id
                ),
                span: s.span,
            },
            StmtKind::Semi(ref e) => Spanned {
                node: hir::StmtKind::Semi(
                    p!(self, self.lower_expr(e)),
                    self.lower_node_id(s.id).node_id
                ),
                span: s.span,
    ...
    fn lower_visibility(
        ...
        let node = match v.node {
            ...
            VisibilityKind::Restricted { ref path, id } => {
                ...
                let def = self.expect_full_def(id);
                hir::VisibilityKind::Restricted {
                    path: p!(self, self.lower_path_extra(
                        def,
                        path,
                        ParamMode::Explicit,
                        explicit_owner,
    ...
    fn arm(
        &mut self,
        pats: hir::HirVec<'a, P<'a, hir::Pat<'a>>>,
        expr: P<'a, hir::Expr<'a>>
    ) -> hir::Arm<'a> {
        hir::Arm {
            attrs: hir_vec![],
    ...
    fn field(&mut self, ident: Ident, expr: P<'a, hir::Expr<'a>>, span: Span) -> hir::Field<'a> {
        hir::Field {
            id: self.next_id().node_id,
            ident,
            span,
    ...
    fn expr_break(&mut self, span: Span, attrs: ThinHirVec<'a, Attribute>) -> P<'a, hir::Expr<'a>> {
        let expr_break = hir::ExprKind::Break(self.lower_loop_destination(None), None);
        p!(self, self.expr(span, expr_break, attrs))
    }
    ...
    fn expr_call(
        ...
        span: Span,
        e: P<'a, hir::Expr<'a>>,
        args: hir::HirVec<'a, hir::Expr<'a>>,
    ) -> hir::Expr<'a> {
        self.expr(span, hir::ExprKind::Call(e, args), thin_hir_vec![])
    }

    fn expr_ident(&mut self, span: Span, ident: Ident, binding: NodeId) -> hir::Expr<'a> {
        self.expr_ident_with_attrs(span, ident, binding, thin_hir_vec![])
    }
    ...
    fn expr_mut_addr_of(&mut self, span: Span, e: P<'a, hir::Expr<'a>>) -> hir::Expr<'a> {
        self.expr(span, hir::ExprKind::AddrOf(hir::MutMutable, e), thin_hir_vec![])
    }
    ...
    fn expr_std_path(
        ...
        span: Span,
        components: &[&str],
        params: Option<P<'a, hir::GenericArgs<'a>>>,
        attrs: ThinHirVec<'a, Attribute>,
    ) -> hir::Expr<'a> {
        let path = self.std_path(span, components, params, true);
        self.expr(
            span,
            hir::ExprKind::Path(hir::QPath::Resolved(None, p!(self, path))),
            attrs,
        )
    }
    ...
    fn expr_match(
        ...
        span: Span,
        arg: P<'a, hir::Expr<'a>>,
        arms: hir::HirVec<'a, hir::Arm<'a>>,
        source: hir::MatchSource,
    ) -> hir::Expr<'a> {
        self.expr(span, hir::ExprKind::Match(arg, arms, source), thin_hir_vec![])
    }
    ...
    fn expr_tuple(
        &mut self,
        sp: Span,
        exprs: hir::HirVec<'a, hir::Expr<'a>>
    ) -> P<'a, hir::Expr<'a>> {
        p!(self, self.expr(sp, hir::ExprKind::Tup(exprs), thin_hir_vec![]))
    }
    ...
    fn stmt_let_pat(
        &mut self,
        sp: Span,
        ex: Option<P<'a, hir::Expr<'a>>>,
        pat: P<'a, hir::Pat<'a>>,
        source: hir::LocalSource,
    ) -> hir::Stmt<'a> {
        let LoweredNodeId { node_id, hir_id } = self.next_id();

        let local = p!(self, hir::Local {
            pat,
            ty: None,
            init: ex,
            id: node_id,
        ...
        respan(sp, hir::StmtKind::Decl(p!(self, decl), self.next_id().node_id))
    }
    ...
    fn block_expr(&mut self, expr: P<'a, hir::Expr<'a>>) -> hir::Block<'a> {
    ...
    fn block_all(
        &mut self,
        span: Span,
        stmts: hir::HirVec<'a, hir::Stmt<'a>>,
        expr: Option<P<'a, hir::Expr<'a>>>,
    ) -> hir::Block<'a> {
        let LoweredNodeId { node_id, hir_id } = self.next_id();
    ...
    fn pat_ok(&mut self, span: Span, pat: P<'a, hir::Pat<'a>>) -> P<'a, hir::Pat<'a>> {
        self.pat_std_enum(span, &["result", "Result", "Ok"], hir_vec![self; pat])
    }

    fn pat_err(&mut self, span: Span, pat: P<'a, hir::Pat<'a>>) -> P<'a, hir::Pat<'a>> {
        self.pat_std_enum(span, &["result", "Result", "Err"], hir_vec![self; pat])
    }

    fn pat_some(&mut self, span: Span, pat: P<'a, hir::Pat<'a>>) -> P<'a, hir::Pat<'a>> {
        self.pat_std_enum(span, &["option", "Option", "Some"], hir_vec![self; pat])
    }

    fn pat_none(&mut self, span: Span) -> P<'a, hir::Pat<'a>> {
    ...
    fn pat_std_enum(
        ...
        span: Span,
        components: &[&str],
        subpats: hir::HirVec<'a, P<'a, hir::Pat<'a>>>,
    ) -> P<'a, hir::Pat<'a>> {
        let path = self.std_path(span, components, None, true);
        let qpath = hir::QPath::Resolved(None, p!(self, path));
    ...
    fn pat_ident(&mut self, span: Span, ident: Ident) -> P<'a, hir::Pat<'a>> {
    ...
    fn pat_wild(&mut self, span: Span) -> P<'a, hir::Pat<'a>> {
        self.pat(span, hir::PatKind::Wild)
    }

    fn pat(&mut self, span: Span, pat: hir::PatKind<'a>) -> P<'a, hir::Pat<'a>> {
        let LoweredNodeId { node_id, hir_id } = self.next_id();
        p!(self, hir::Pat {
            id: node_id,
            hir_id,
    ...
    fn std_path(
        ...
        let mut path = self.resolver
            .resolve_str_path(self.arena, span, self.crate_root, components, is_value);
        let mut segments = path.segments.into_vec();
    ...
    fn ty_path(&mut self, id: LoweredNodeId, span: Span, qpath: hir::QPath<'a>) -> hir::Ty<'a> {
        let mut id = id;
    ...
    fn elided_path_lifetimes(&mut self, span: Span, count: usize) -> P<'a, [hir::Lifetime]> {
        let arena = self.arena;
    ...
    fn wrap_in_try_constructor(
        ...
        let from_err = p!(self, self.expr_std_path(unstable_span, path, None,
                                            thin_hir_vec![]));
        p!(self, self.expr_call(e.span, from_err, hir_vec![self; e]))
    }
}
